...
2.  **Configure your environment**
    ...
    # Public origin routed to the status-page container. Slug pages are
    # published below it, for example https://status.example.com/acme.
...
-   **Status Pages:** `http://localhost:3003`
    ...
    `acme` is published at `https://status.example.com/acme`. To use wildcard
    subdomains instead, set `STATUS_PAGE_URL=https://{slug}.status.example.com`
    and configure wildcard DNS and proxy routing.

    For a custom domain such as `status.acme.com`, point its DNS at your reverse
    proxy, terminate TLS there, and forward it to the status-page container on
    port 3000. Preserve the original `Host` header or set `X-Forwarded-Host` to
    it; openstatus uses that hostname to find the matching page.
